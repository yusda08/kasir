...
class Setting extends MY_Controller {
    ...
    public function set_user() {
        if ($this->session->userdata('is_login')) {
            ...
            $record['get_levelUser'] = $this->Model_setting->get_levelUser();
    ...
    public function set_profil() {
        if ($this->session->userdata('is_login')) {
            ...
            $record = $this->javasc();
            $data['name_page'] = 'Setting';
            $data['name_page_small'] = 'Profil';
            $record['num_row'] = $this->Model_setting->get_profil()->num_rows();
            $record['row_pro'] = $this->Model_setting->get_profil()->row();
            $data['content'] = $this->load->view('setting/set_profil', $record, TRUE);
            $this->load->view('temp_home/layout', $data);
        } else {
            redirect('login');
        }
    ...
    function insert_profil() {
        ...
        $query = $this->Model_aksi->insert('profil', $data);
        if ($query) {
            $this->session->set_flashdata('tipe', 'alert-success');
            $this->session->set_flashdata('msg', 'Berhasil Disimpan');
            $ket = 'Menambah Data Profil';
            $this->aktifitas($ket);
            redirect($url);
        } else {
